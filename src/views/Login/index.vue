  ...
  <div :class="['login wow animated',loginActive?'intoOut':'into']">
    <div class="login_warp" >
      ...
      <el-form :model="userinfo" status-icon :rules="rules" ref="ruleForm" class="login_form" size="mini">
        ...
        <el-form-item>
            <el-button :disabled="btnstatus"  class="mybtn" type="info"
            style="width:100%;"  @click="submitForm('ruleForm')">
              {{model === 'login' ? '登录' : '注册'}}
            </el-button>
        </el-form-item>
       </el-form>
...
import { reactive, ref, isRef,computed, onMounted } from '@vue/composition-api';
export default {
  name: "login",
  setup(props, { refs, root }){
      root.$nextTick(() => { // 在dom渲染完后,再执行动画
